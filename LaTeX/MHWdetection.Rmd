...
The effect of a long-term trend on MHW detection was the most linear of the three tests and resulted in the largest changes in the metrics. Very rarely, an added linear trend led to a reduction in the count of average MHWs in a time series, but generally it caused a linear increase at roughly xxx% additional average MHWs detected for every 0.01°C/dec added (Figure 2G). The effect that these additional MHWs had on the sum of average MHW days was an increase, ranging from 1.8 -- 9.0% for every 0.01°C/dec added (Figure 2H). The effect of linear trends on the maximum intensity of the average MHWs, though generally linear, could be either positive or negative at a rate of -0.9 -- 0.7% per 0.01°C/dec added.

The focus MHW in each time series was never divided into multiple events due to the added long-term trend (Figure 3G) however; the duration of the focus MHWs were affected differently. The Mediterranean focus MHW showed practically no increase in duration due to an added long-term trend, the Western Australia MHW saw a large jump at 0.03°C/dec, and the Northwest Atlantic MHW had a dramatic jump at an added trend of 0.09°C/dec, followed by a few other increases at larger added trends (Figure 3H). Likewise, all of the other 1000 time series included in Figure 3 tend to jump up in dramatic steps, as seen by the very large range in the 90th and 50th percent confidence intervals. These jumps in duration occur when the temperature anomalies are increasing more rapidly than the threshold and neighbouring MHWs in a time series will connect into one event. The effect that the long-term trend had on the maximum intensity of each event was a simple linear function of the added trend and where in the time series the event occurred (Figure 3I).
...
Figure 7: The effect of changing window widths on MHW output as a fix for time series length. RWS: I have these results but haven't made this figure yet as I don't think it is necessary.

Supplementary 1: The effect of different 30 year climatology base periods on MHW results. RWS: I haven't created this yet as I'm not certain it is necessary.

Supplementary 2: The effect of the sub-optimal tests on the seasonal mean and threshold climatologies. These panels show the same information as Figure 3 except that the variables shown are the thresholds from the MHW detection output.

Supplementary 3: The same global map as Figure 4, but with rates of change shown due to increasing missing data from 0 -- 50%.

Supplementary 4: The same global map as Figure 4, but with rates of change shown due to increasing decadal trends from 0.00 -- 0.30°C/dec.
