...
<!-- RWS: Need to state up front where the code may be accessed. -->
...
<!--     - For example, when a time series has a long-term trend of 0.2°C/dec in an eastern boundary upwelling system (EBUS), does the effect this have on the duration of the average MHWs differ from the same trend in a time series in a western boundary current (WBC)? -->
...
While not a specific focus in this study, the effects that the sub-optimal tests have on the seasonal mean and threshold climatologies have been included in the supplementary material (Figure S1).
...
To quantify the effect of a long-term (secular) trend on the MHW results we add linear trends of 0.00 -- 0.30°C/dec in 0.01°C/dec steps to each time series. The control time series are those with no added trend (e.g. 0.00°C/dec).
...
The effect of random missing data on the focus MHW in each time series was dramatic. As missing data in a time series increased, it becomes increasingly likely that the focus MHW is broken into multiple smaller events. It is not uncommon for this to begin with as little as 1% missing data, and increases in severity up to 25 -- 30% (Figure 3D). From this point the number of separate events the MHW is broken into decreases as the smaller events are completely missed due to the loss of data. The duration of the focus MHW was almost always negatively impacted by missing data (Figure 3E). The decrease in duration follows a linear trend of a reduction ranging from 1 -- 3% per 1% of missing data. At 26% missing data at least 5% of the time series had their focus MHW removed entirely from the time series, as seen by a reduction in maximum intensity of 100% (Figure 3F). At 41% missing data at least 25% of the time series had their focus MHW removed. 

<!-- RWS: Need a sentence in the above aparagraph discussing the effect of missing data on the peaks of pointy MHWs. -->
...
The global patterns in missing data are unremarkable and generally consistent across the oceans. They may be seen in the supplementary material (Figure S3).
...
1) Time series length:
  ...
  - The shortening of a time series introduced the largest uncertainty into the duration of average and focus MHW:
    - Average MHW duration changes by -1.62 -- 3.8%/year shorter than 30 (90% CI)
    - Focus MHW duration changes by -2.16 -- 1.05%/year shorter than 30 (90% CI)
  - Smoothing the climatology before detecting MHWs does not improve the results and should not be done  
2) Missing data:
  - The effect of missing data up to 25% on MHW results was comparable to the effect of a 10 year time series
  - Focus MHWs may begin to disappear from time series missing 26% or more data
...
3) Long-term trends
  - Long-term trends had a greater effect on MHWs than the other two sub-optimal tests and had a greater range of uncertainty around those effects
...
Figure S1: The effect of the sub-optimal tests on the seasonal mean and threshold climatologies. These panels show the same information as Figure 2 except that the variables shown are the thresholds from the MHW detection output.

Figure S2: The effect of different 30 year climatology base periods on MHW results. The elements of this figure are the same as Figure 2.

Figure S3: The same global map as Figure 4, but with rates of change shown due to increasing missing data from 0 -- 50%.

Figure S4: The same global map as Figure 4, but with rates of change shown due to increasing long-term trends from 0.00 -- 0.30°C/dec.

Figure S5: The effect of increasing window half width on the focus MHWs. This figures contains the same information as Figure 5 but shows the focus MHW results rather than the average MHW results.
