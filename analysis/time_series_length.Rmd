...
<!-- RWS: Reviewer 2 shot down this methodological appraoch to random time series creation for a number of reasons. Rather it is now proposed that real time series from the OISST dataset be used instead. Which then begs the question, why not just use all of the OISST pixels and report on what shakes out? -->
...
- Confidence (CIs) for each doy seas/thresh and MHW metrics at different time series lengths will be calculated
- Correlations between time series length and seas/thresh mean/median and MHW metrics
...
Below we systematically shorten the three pre-packaged time series available in the __`heatwaveR`__ package and the python distribution from 37 -- 3 years on the first de-trended data. Eventually this will be run on the global data.
...
```{r shortening-time-series, eval = FALSE}
...
It emerged in an earlier version of this manuscript that the 30 year period within a time series chosen for the climatology may have a noticeable impact on the MHW results. To this end we will also compare the results from our three default time series with varying 30 year climatology periods.
