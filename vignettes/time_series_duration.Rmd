...
Two different shortening methodologies are proposed here. The first would be to simply take the last n years in a given time series (e.g. 30, 20, 10) and then compare the detected events in the overlapping period of time.
[ECJO]
A key importance here will be if there is any long term trend in the data, let’s say long-term warming.  If so then even if 10 yrs produced an adequate climatology, going longer (20yrs, 30yrs) would produce events with larger intensities and longer means since the climatology would be calculated over an on-average cooler time period.
[/ECJO]
...
[ECJO]
I can’t interpret this from the above…  I’m not even sure what the above is.  Why is duration always <1, what are the units? This needs some explanation….*confused*
[/ECJO]
...
```
[ECJO]
I think we should focus on the most used metrics i.e. duration and intensity (mean, max, cumulative)
[/ECJO]
...
```{r}
# RWS: I'm putting htis on hold for now until I've editted the text here and in the skeleton.
```
