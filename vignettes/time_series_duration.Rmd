...
# Function for artificially correcting the years of the re-sampled time series
# df <- sst_ALL_repl %>% 
...
parse_date_sst <- function(df){
  parsed <- df %>% 
    ...
    # summarise(n())
    mutate(y = seq(unique(year_index), 2018)) %>%
...
sample_30 <- function(df){
  ...
  levels(year_filter) <- as.character(year_filter)
  year_filter <- data.frame(year = as.character(year_filter), stringsAsFactors = F)
  res <- df %>% 
    mutate(year = as.character(year(t))) %>% 
    right_join(year_filter, by = "year") %>% 
    mutate(year_original = year,
           month = month(t),
           day = day(t),
           year = factor(year, levels = 1989:2018),
           raar = as.character(levels(year))) #%>% 
    # mutate(t = as.Date(fastPOSIXct(paste(y, m, d, sep = "-"))))
  return(res)
}
