...
rm(sst_ALL_smooth_aov_tukey, sst_ALL_smooth_aov, label_count_aov_p)
```
...
ggplot(clim_tukey_sig) +
  # geom_histogram(bins = 37, fill = "firebrick") +
  geom_errorbarh(aes(xmin = year_long, xmax = year_short, 
                     y = year_index)) +
  geom_label(aes(x = year_dif/2, y = year_index, label = count)) +
  labs(x = "Difference in years") #%>%
  # facet_wrap(~site)
  # facet_grid(rows = vars(metric), 
  #            cols = vars(site), scales = "fixed")

ggplot(clim_tukey_sig, aes(x = year_dif, y = year_long)) +
  geom_point() +
  facet_wrap(~site) +
  scale_x_reverse()
...
rm(sst_ALL_smooth, sst_ALL_smoooth_cor_base)
```
