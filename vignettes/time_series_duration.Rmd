...
# Function to remove the simple linear trend from a time series
detrend <- function(df){
...
# Calculate decadal trends and remove them
# df <- sst_ALL_repl %>%
  # filter(site == "WA", rep == "32")
detrend <- function(df) {
  
  # Create month and year columns for better trend detection
  df2 <- df %>% 
    mutate(year = year(t),
           month = month(t)) %>% 
    group_by(year, month) %>%
  ...
  # Run the gls model from Schlegel 2016
  gls_res <- gls(temp ~ num, correlation = corARMA(form = ~ 1 | year, p = 2),
                 method = "REML", data = df2, na.action = na.exclude)
  
  # The decadal trend from the resampled data
  dt <- round(as.numeric(coef(gls_res)[2] * 120), 3)
  
  # The gls residuals
  gls_fitted <- data.frame(year = df2$year, 
                           month = df2$month,  
                           fitted = as.vector(gls_res$fitted))
  
  # Combine results and exit
  res <- df %>%
    mutate(year = year(t),
           month = month(t)) %>% 
    left_join(gls_fitted, by = c("year", "month")) %>% 
    mutate(anom_flat = temp - fitted) %>% 
    select(t, anom_flat) %>% 
    dplyr::rename(temp = anom_flat)
  return(res)
}

# testing the old vs. ne methods
test <- rbind(res, res_old[,1:2])
test$test <- rep(c("new", "old"), each = nrow(res))

ggplot(data = test, aes(x = t, y = temp, colour = test)) +
  geom_line() +
