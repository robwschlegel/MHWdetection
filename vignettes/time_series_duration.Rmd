...
The time series shortening technique proposed here is [re-sampling](https://robwschlegel.github.io/MHWdetection/articles/Short_climatologies.html). This methodology takes the full 37 year de-trended time series and randomly samples _n_ years from it to simulate a 10 -- 30 year time series. One then uses the jumbled up, randomly selected years of data to create the seasonal signal and 90th percentile threshold (hereafter referred to as "the climatologies"). The real temperature data, in the correct day-of-year (doy) order, are still used against the re-sample created climatologies. Re-sampling the time series in this way is useful because it allows us to replicate random climatology creation 100 (or more) times for each time series length in order to produce a more confident estimation of how likely climatologies generated from certain time series lengths are to impact the accuracy of event detection.

The WMO recommends that the period used to create a climatology be 30 years in length, starting on the first year of a decade (e.g. 1981), however; because we are going to be running these analyses on time series of many different lengths, it will be more consistent to use all of the years present in each as the climatology period. For example, if a time series spans 1989 -- 2018, that will be the period used for calculating the climatologies. Likewise, should the time series only span the years 2006 -- 2018, that will be the period used.


## Re-sampling

In this section we will compare the detection of events when we shorten time series using [re-sampling](https://robwschlegel.github.io/MHWdetection/articles/Short_climatologies.html) to simulate 100 time series at each length of 10 -- 30 year. This is presently being done with the three pre-packaged time series available in the __`heatwaveR`__ package and the python distribution, but eventually will be run on the global data.
...
# Function for creating a re-sample from 30 years of data
sample_30 <- function(df){
  year_filter <- c(1,2,3)
  while(length(unique(year_filter)) != 30){
    year_filter <- factor(sample(1982:2018, size = 30, replace = F))
  }
  res <- data.frame()
...
sst_ALL_repl <- purrr::rerun(100, sample_30(sst_ALL_detrend)) %>%
...
# Run this with preferably 21 cores for speed
doMC::registerDoMC(cores = 11)
sst_ALL_smooth <- plyr::ldply(1989:2009, shrinking_results, .parallel = T)

save(sssst_ALL_smooth, file "data/sst_ALL_smooth.Rdata")
```
...
sst_ALL_smooth_aov <- plyr::ddply(sst_ALL_smooth, c("rep", "site"), clim_aov, .parallel = T)
save(sst_ALL_smooth_aov, file = "data/sst_ALL_smooth_aov.RData")
```

```{r clim-aov-box, fig.cap="Boxplots showing the range of _p_-values from ANOVA's for the different lengths of the 100 re-samplings for the three reference time series. A label at the bottom of the boxplot shows the number of re-samples that had significantly different results."}
load("data/sst_ALL_smooth_aov.RData")
...
ggplot(sst_ALL_smooth_aov, aes(x = metric, y = p.value, fill = metric)) +
  geom_boxplot() +
  # geom_jitter(alpha = 0.2, width = 0.1, aes(group = metric)) +
  geom_label(data = label_count_aov_p, fill = "white",
             aes(y = -0.1, label = count), show.legend = F) +
  scale_fill_viridis_d() +
  labs(y = "p-value", x = NULL) +
  facet_wrap(~site) +
...
The above figure shows that there are no significant differences in the climatologies for the Mediterannean or North West Atlantic time series. We do however see that of the 100 re-samples for the Western Australia time series, seven of the seasonal signals and 23 of the thresholds did differ significantly at some point during the shortening.

### Post-hoc Tukey test

```{r clim-tukey-res, eval=FALSE}
sst_ALL_smooth_tukey <- plyr::ddply(sst_ALL_smooth, c("rep", "site"), clim_tukey, .parallel = T)
save(sst_ALL_smooth_tukey, file = "data/sst_ALL_smooth_tukey.RData")
```
...
load("data/sst_ALL_smooth_tukey.RData")

clim_tukey_sig <- sst_ALL_smooth_tukey %>% 
  filter(adj.p.value <= 0.05) %>% 
  separate(comparison, into = c("year_short", "year_long"), sep = "-") %>% 
  mutate(year_short = as.numeric(year_short),
         year_long = as.numeric(year_long))

clim_tukey_count <- clim_tukey_sig %>% 
  mutate(year_dif = year_short - year_long)

ggplot(clim_tukey_count, aes(x = year_dif)) +
  geom_histogram(bins = 16, fill = "firebrick") +
...
sst_ALL_smooth_cor_base <- sst_ALL_smooth %>% 
  ...
  group_by(rep, site, year_index) %>% 
  summarise(length = 2018-year_index[1]+1,
...
The above figure shows us that all three reference time series respond relatively similarly with regards to the correlations of their climatology means and SD to the length and SD of the base temperatures from which the climatologies were derived. Though the patterns observed otherwise tell us little, it is an important finding to know that this dud pattern is consistent across the different reference time series. The other important pattern this figure shows is that the length of a time series appears to not have a consistent relationship with the resultant climatologies. Instead we see that it is the variance in the time series that tends to have a positive relationship with the variance in the climatologies.

The pattern that is beginning to emerge from the results for the climatologies is that what matters most for the difference in the results is how much variance is present in the base time series. This variance is a representation of the presence of extreme temperature differences. Relating to the reseach question here this means large MHWs. It must therefore be investigated what the relationship is between categories of MHWs and the significant differences detected in the thresholds in the WA time series. It should also be taken into consideration that wheneber the re-sampled time series have the year 2011, the massive MHW, these will likely be the ones producing significantly different results.


## MHW metrics

With the effects of time series duration on the climatologies now mapped out it is ime to apply a similar methodology to the MHW metrics.
...
ggplot(sst_ALL_event_aov, aes(x = metric, y = p.value, fill = metric)) +
  geom_boxplot() +
  geom_label(data = label_count_event_aov_p, fill = "white",
             aes(y = -0.1, label = count), show.legend = F) +
  scale_fill_viridis_d() +
  labs(y = "p-value", x = NULL) +
  facet_wrap(~site) +
...
The above figure shows us that generally the length of a time series has no significant effect on the MHWs detected within it. We see that the MHWs in the Mediterranean time series are not affected at all in any of the 100 replicates. Surprisingly the events in the North West Atlantic time series are significantly different more often than the Western Australia time series. This must mean that the event metrics are more sensistive to a number of longer larger events than a single large one, as is the case in the WA time series. This also means that the thresholds between shortened time series do not need to be significantly different in order to produce significantly different events. It must now be investigate just which years are significantly different from one another.
...
event_tukey_count <- event_tukey_sig %>% 
  mutate(year_dif = year_short - year_long)

ggplot(event_tukey_count, aes(x = year_dif)) +
  geom_histogram(bins = 6, fill = "firebrick") +
  labs(x = "Difference in years")
```

The above figure shows us that of all the significantly different ANOVA results, when we run the post-hoc Tukey test only six of the pairwise comparisons produce significantly different MHW metrics, and they are all `mean intensity`. Five of these six results were from run 55 and only became significantly different from a difference in years of 16 -- 20.


### Correlations

```{r event-core-base, eval=FALSE}
# Summary stats for correlation
...
sst_ALL_event_cor <- sst_ALL_smooth %>% 
  ...
  group_by(rep, site, year_index) %>% 
  mutate(length = 2018-year_index[1]+1) %>% 
  group_by(site, rep, length) %>% 
  select(-year_index) %>% 
...
The figure above shows us that the relationship between the length of all three reference time series and the resultant MHW metrics are similar, though weakest for the NW Atl. Specifically we see that mean/sd duration of events has a strong (r~0.5) relationship to the length of the time series. Meaning that longer duration time series will tend to have MHWs with greater duration. This was to be expected as the initial hypothesis was that longer time series would have lower thresholds and so events detected with them would be longer and more intense. We see then that there is also generally a positive relationship between time series length and cummulative intensity mean/sd. This relationship breaks down a bit ith max intensity, and begins to appear negative for mean intensity.
...
ggplot(sst_ALL_clim_category_count, aes(x = length, y = count, fill = category)) +
