...
# year_index <- 3
KS_sub <- function(df_1, df_2){
  # df_1 <- sst_ALL_clim_only %>% 
    # filter(year_index == 30)
  # df_2 <- df #%>% 
    # filter(year_index == year_index)
  ...
  res <- data.frame(seas = round(ks.test(df_1$seas, df_2$seas)$p.value, 4),
                    ...
                    year_1 = 30,
                    year_2 = df$year_index[1])
  )
  return(res)
}

# rep_count <- 3
KS_all <- function(rep_count){
  # df_30
  df_main <- sst_ALL_clim_only %>% 
    # filter(as.numeric(rep) == rep_count) %>% 
    mutate(year_index2 = year_index) %>%
    # group_by(site, rep, year_index2) %>%
    group_by(year_index2) %>% 
    nest() %>% 
    mutate(KS = map(data, KS_sub)) %>% 
    select(-data, -year_index2) %>% 
    unnest() %>% 
    tidyr::gather(key = "metric", value = "p.value", -site, -rep, -year_1, -year_2)
  return(df_main)
}

doMC::registerDoMC(cores = 50)
sst_ALL_KS <- plyr::ldply(1:100, KS_all, .parallel = T)
