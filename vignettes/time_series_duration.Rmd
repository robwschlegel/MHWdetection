...
#   res <- ts2clm(df, climatologyPeriod = c(paste0(year_start,"-01-01"), "2018-12-31")) %>%
...
shrinking_results <- function(year_begin){
  sst_ALL_res <- sst_ALL_repl %>% 
    ...
    nest() %>% 
    mutate(clims = map(data, ts2clm, climatologyPeriod = c(paste0(year_begin,"-01-01"), "2018-12-31")),
...
```{r clim-aov-box, fig.cap="Boxplots showing the range of _p_-values from ANOVA's for the different lengths of the 100 re-samplings for the three reference time series."}
...
ggplot(sst_ALL_smooth_aov, aes(x = site, y = p.value, fill = metric)) +
  geom_boxplot() +
  # geom_jitter(alpha = 0.2, width = 0.1, aes(group = metric)) +
  geom_label(data = label_count_aov_p, fill = "white",
             aes(y = -0.1, label = count, group = metric), show.legend = F,
...
```{r event-aov-box, fig.cap="Boxplots showing the range of _p_-values from ANOVA's for the different lengths of the 100 re-samplings for the three reference time series."}
...
# ANOVA p
sst_ALL_smooth_event_aov_p <- sst_ALL_smooth_event %>% 
  select(site, rep, duration, intensity_mean, intensity_max, intensity_cumulative) %>% 
  nest(-site, -rep) %>%
  mutate(res = map(data, aov_p)) %>% 
  select(-data) %>% 
  unnest() #%>% 
...
# The top 5 events from 30 years
