...
In order to detemine if the count of MHWs, both total and individual categories, differs based on the percent of missing data, we will be performing a series of _chi_-squared tests for each site and rep.

```{r miss-chi-calc, eval=FALSE}
# Simple wrapper for nesting
# df <- sst_ALL_miss_cat_all %>%
  # filter(site == "WA", rep == "1")
chi_p <- function(df){
  res <- chisq.test(table(df$miss, df$category), correct = FALSE)
...
sst_ALL_miss_cat_total <- sst_ALL_miss_cat %>% 
  select(miss, rep, site, category) %>% 
  mutate(category = "total")

sst_ALL_miss_cat_all <- sst_ALL_miss_cat %>% 
  select(miss, rep, site, category) %>% 
  # rbind(sst_ALL_miss_cat_total) %>% 
...
sst_ALL_miss_cat_chi <- sst_ALL_miss_cat_all %>% 
  group_by(site, rep) %>% 
  nest() %>% 
...
ggplot(chi_sig, aes(x = as.numeric(miss), y = site)) +
