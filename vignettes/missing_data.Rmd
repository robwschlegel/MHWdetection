...
# Randomly knockout 1 - 50% of data
# The knockout_repl() function repeats random_knockout() 100 times
# for each step in the missing data progression
doMC::registerDoMC(cores = 50)
sst_ALL_miss <- plyr::ldply(seq(0.01, 0.50, 0.01), knockout_repl, .parallel = T)

# Add the original data as rep = "0" and miss = "0.00" 
sst_ALL_0 <- sst_ALL_detrend %>% 
  mutate(miss = "0.00",
         rep = "0") %>% 
  select(miss, rep, everything())

sst_ALL_miss <- rbind(sst_ALL_0, sst_ALL_miss)
...
doMC::registerDoMC(cores = 50)
sst_ALL_con <- plyr::ddply(filter(sst_ALL_miss, miss != "0.00"), 
...
ggplot(clim_tukey_sig, aes(x = miss_small, y = miss_large)) +
  geom_point() +
  # geom_errorbarh(aes(xmin = year_long, xmax = year_short)) +
  # geom_tile() +
  # geom_segment(aes(xend = miss_small, 
  #                  y = min(miss_large), yend = max(miss_large))) +
  facet_grid(~site) +
  # scale_x_reverse() +
...
The above figure tends to show 
...
KS_sig <- rbind(KS_sig, data.frame(site = "Med", metric = "thresh", perc = 1:26, count = NA))
KS_sig <- rbind(KS_sig, data.frame(site = "NW_Atl", metric = "thresh", perc = 1:21, count = NA))
KS_sig <- rbind(KS_sig, data.frame(site = "WA", metric = "thresh", perc = 1:8, count = NA))
...
sst_ALL_miss_cor <- sst_ALL_miss_joined %>% 
  ...
  do(data.frame(t(cor(.[,6:25], .[,5], use = "pairwise.complete.obs")))) %>% 
...
```{r miss-event-aov-calc, eval=FALSE}
