...
random_knockout <- function(prop){
  ...
  ts_length <- nrow(filter(sst_ALL_detrend, site == "WA"))
...
knockout_repl <- function(prop) {
  res <- purrr::rerun(100, random_knockout(prop)) %>%
    map_df(as.data.frame, .id = "rep") %>% 
    select(miss, rep, everything())
  return(res)
}
...
sst_ALL_miss <- plyr::ldply(seq(0.00, 0.50, 0.01), knockout_repl, .parallel = T)
...
sst_ALL_con_plot <- sst_ALL_con %>% 
  ...
  ungroup() %>% 
  mutate(prop = round((1-as.numeric(miss))*100))
...
# Line plot
ggplot(sst_ALL_con_plot, aes(x = prop, y = duration, colour = count)) +
...
```{r miss-clim-event-cat-calc, eval=TRUE}
# This file is not uploaded to GitHub as it is too large
# One must first run the above code locally to generate and save the file
load("data/sst_ALL_miss.Rdata")
...
ggplot(filter(sst_ALL_miss_clim_only, rep %in% as.character(1:10)), 
       aes(y = seas, x = doy, colour = ((1-as.numeric(miss))*100))) +
...
ggplot(filter(sst_ALL_miss_clim_only, rep %in% as.character(1:10)), 
       aes(y = thresh, x = doy, colour = ((1-as.numeric(miss))*100))) +
  geom_line(alpha = 0.3, aes(group = interaction(rep, miss))) +
  scale_colour_viridis_c() +
  facet_wrap(~site, ncol = 1, scales = "free_y") +
  labs(x = "Calendar day of year", y = "Temp. (°C)", colour = "Missing (%)")
...
load("data/sst_ALL_miss_clim_only.Rdata")
doMC::registerDoMC(cores = 50)
sst_ALL_miss_clim_aov_tukey <- plyr::ddply(sst_ALL_miss_clim_only, c("site"), 
...
sst_ALL_miss_clim_CI <- sst_ALL_miss_clim_only %>% 
  ...
  select(-data) %>% 
  unnest() %>% 
  mutate(prop = round(1-as.numeric(miss), 2),
         prop = ifelse(prop == 1.00, 0.00, prop))

ggplot(filter(sst_ALL_miss_clim_CI, metric != "seas"), 
       aes(x = as.factor(prop*100))) +
...
```{r miss-clims-KS-calc, eval=TRUE}
...
KS_sig <- sst_ALL_miss_clim_KS_p %>% 
  ...
  mutate(prop = round(1-as.numeric(as.character(miss)), 2),
         perc = prop*100) %>% 
  select(site, metric, count, perc)
...
ggplot(KS_sig, aes(x = perc, y = site)) +
...
```{r clim-cor, fig.cap="Dot plot showing the relationship between number of consecutive missing days and the significant difference of that climatology as determined by KS tests."}
...
sst_ALL_miss_cor <- sst_ALL_miss_joined %>% 
  ...
  do(data.frame(t(cor(.[,6:ncol(sst_ALL_miss_joined)], .[,5], use = "pairwise.complete.obs")))) %>% 
...
```{r miss-event-aov-calc, eval=TRUE}
...
load("data/sst_ALL_miss_clim_event_cat.Rdata")
doMC::registerDoMC(cores = 50)
sst_ALL_miss_event_aov_tukey <- plyr::ddply(sst_ALL_miss_clim_event_cat, c("site"), 
...
# visualise
ggplot(sst_ALL_miss_event_aov_p, aes(x = site, y = metric)) +
  geom_tile(aes(fill = p.value)) +
...
As we may see in the figure above, all of the metrics differ significantly when various amounts of missing data are introduced into the time series. To see where these differences materialise we will visualise the post-hoc Tukey test results.


### Post-hoc Tukey test

```{r event-tukey-heat, fig.cap="Segments showing the range of the percent of missing data present when climatologies were significantly different. The x and y axes show the percent of missing data."}

sst_ALL_miss_event_tukey <- sst_ALL_miss_event_aov_tukey %>% 
  select(-aov) %>% 
  unnest()

event_tukey_sig <- sst_ALL_miss_event_tukey %>% 
  filter(adj.p.value <= 0.05) %>% 
...
# Need to correct for the tukey putting the 0% missing data on the wrong side
event_tukey_sig_fix <- event_tukey_sig %>% 
  filter(miss_dif < 0) %>% 
  mutate(miss_dif = abs(miss_dif))
colnames(event_tukey_sig_fix)[3:4] <- c("miss_large", "miss_small")

# Apend the corrected data
event_tukey_sig <- event_tukey_sig %>% 
  filter(miss_dif > 0) %>% 
  rbind(event_tukey_sig_fix)

# Create a summary for better plotting
event_tukey_summary <- event_tukey_sig %>% 
  group_by(site, metric, miss_small) %>% 
  summarise(range_bottom = min(miss_large),
            range_top = max(miss_large))
  # group_by(site, metric, miss_small) 
...
ggplot(event_tukey_summary, aes(x = miss_small)) +
  # geom_point() +
  # geom_errorbarh(aes(xmin = year_long, xmax = year_short)) +
  # geom_tile() +
  geom_segment(aes(xend = miss_small,
